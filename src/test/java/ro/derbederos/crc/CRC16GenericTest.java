...
public class CRC16GenericTest {
    ...
    public CRC16GenericTest(CRCParameters crcParameters) {
    ...
    public void testResidue() {
        ...
        short input = (short) crcParameters.getXorOut();
        if (crcParameters.getRefOut()) {
            //TODO: hack, fixes issue with CRC-5/USB
            input = (short) (Util.reverseShort(input) >>> 16 - Util.roundToByte(crcParameters.getWidth()));
        }
        byte[] newByte = Util.shortToBytes(input);
        int len = Util.roundToByte(crcParameters.getWidth()) / 8;
